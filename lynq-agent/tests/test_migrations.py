...
class JdbcTargetTest(unittest.TestCase):
    ...
    def test_the_sqlalchemy_url_becomes_a_jdbc_url_with_split_credentials(self) -> None:
    ...
    def test_a_missing_port_falls_back_to_the_mysql_default(self) -> None:
        target = jdbc_target("mysql+aiomysql://root:secret@mysql/lynq_agent_db")

        self.assertEqual(target.url, "jdbc:mysql://mysql:3306/lynq_agent_db")

    def test_a_percent_encoded_password_is_decoded_for_the_driver(self) -> None:
        target = jdbc_target("mysql+aiomysql://root:p%40ss%3Aword@mysql:3306/db")

        self.assertEqual(target.password, "p@ss:word")

    def test_connection_options_survive_the_translation(self) -> None:
    ...
    def test_a_url_without_a_database_is_rejected(self) -> None:
        with self.assertRaises(MigrationError):
            jdbc_target("mysql+aiomysql://root:secret@mysql:3306/")

    def test_a_url_without_a_host_is_rejected(self) -> None:
        with self.assertRaises(MigrationError):
...
class LiquibaseExecutableTest(unittest.TestCase):
    ...
    def test_the_image_installation_is_preferred(self) -> None:
    ...
    def test_a_developer_install_on_the_path_is_used_when_there_is_no_home(self) -> None:
        with patch.dict("os.environ", {}, clear=True):
            with patch("db.migrations.os.access", return_value=False):
                with patch(
                    "db.migrations.shutil.which",
                    return_value="/usr/local/bin/liquibase",
                ):
                    self.assertEqual(liquibase_executable(), "/usr/local/bin/liquibase")

    def test_the_failure_names_what_the_developer_has_to_install(self) -> None:
        with patch.dict("os.environ", {}, clear=True):
            with patch("db.migrations.os.access", return_value=False):
...
class UpdateTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        executable = patch(
            "db.migrations.liquibase_executable",
            return_value="/opt/liquibase/liquibase",
        )
        executable.start()
    ...
    def test_the_changelog_and_search_path_are_passed_to_liquibase(self) -> None:
        with patch("db.migrations.subprocess.run") as run:
            run.return_value = subprocess.CompletedProcess([], 0, "", "")
            _update_blocking(COMPOSE_URL)
    ...
    def test_usage_analytics_are_off_so_no_schema_detail_leaves_the_container(self) -> None:
        with patch("db.migrations.subprocess.run") as run:
            run.return_value = subprocess.CompletedProcess([], 0, "", "")
            _update_blocking(COMPOSE_URL)
    ...
    def test_a_failed_update_reports_what_liquibase_printed(self) -> None:
        with patch("db.migrations.subprocess.run") as run:
    ...
    def test_a_failed_update_does_not_echo_the_password(self) -> None:
        with patch("db.migrations.subprocess.run") as run:
...
class UpdateToLatestTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_broken_migration_stops_the_service_from_booting(self) -> None:
        with patch("db.migrations._update_blocking", side_effect=MigrationError("boom")):
            with self.assertRaises(MigrationError):
                await update_to_latest()

    async def test_a_healthy_migration_lets_the_service_boot(self) -> None:
