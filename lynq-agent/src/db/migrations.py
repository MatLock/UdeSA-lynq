...
def jdbc_target(db_url: str) -> JdbcTarget:
    ...
    port = parts.port or DEFAULT_MYSQL_PORT
    database = parts.path.lstrip("/")
    if not database:
        raise MigrationError("DB_URL has no database name")

    url = f"jdbc:mysql://{parts.hostname}:{port}/{database}"
    if parts.query:
...
def liquibase_executable() -> str:
    home = os.getenv("LIQUIBASE_HOME", DEFAULT_LIQUIBASE_HOME)
    bundled = os.path.join(home, "liquibase")
